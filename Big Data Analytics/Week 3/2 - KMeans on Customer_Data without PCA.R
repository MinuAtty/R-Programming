...
ggplot(data, aes(x = Annual_Income, y = Age, color = Cluster)) +
  ...
  labs(title = "Age vs Annual_Income distribution of clusters",  # Title for the plot
       x = "Annual_Income",  # X-axis label
       y = "Age") +  # Y-axis label
...
ggplot(data, aes(x = Spending_Score, y = Age, color = Cluster)) +
  stat_ellipse(aes(fill = Cluster), geom = "polygon", alpha = 0.4) +  # Draw cluster boundaries with transparency
  geom_point(size = 3, alpha = 0.7) +  # Plot data points with slight transparency
  labs(title = "Age vs Spending_Score distribution of clusters",  # Title for the plot
       x = "Spending_Score",  # X-axis label
       y = "Age") +  # Y-axis label
...
ggplot(data, aes(x = Annual_Income, y = Spending_Score, color = Cluster)) +
  stat_ellipse(aes(fill = Cluster), geom = "polygon", alpha = 0.4) +  # Draw cluster boundaries with transparency
  geom_point(size = 3, alpha = 0.7) +  # Plot data points with slight transparency
  labs(title = "Annual_Income vs Spending_Score distribution of clusters",  # Title for the plot
       x = "Annual_Income",  # X-axis label
       y = "Spending_Score") +  # Y-axis label
